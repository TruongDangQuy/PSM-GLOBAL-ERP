...
#Region "Link_ISUD"
    Public Function Link_ISUD4064A(job As Integer, cdMainProcess As String, Optional CheckName As String = "") As Boolean
        If strFormName = "" Then strFormName = Me.Text
        Me.Tag = CheckName

        D4064.cdMainProcess = cdMainProcess
        ...
        wJOB = job : L4064 = D4064

        Link_ISUD4064A = False
...
#Region "DATA_SEARCH"
    ...
    Private Function DATA_SEARCH_HEAD() As Boolean
        ...
        'DS1 = READ_PFK4064(L4064.cdMainProcess, cn)
    ...
    Private Sub DATA_SEARCH_LINE(Type As String, XROW As Integer, ByVal ParamArray strPara() As String)
        Try
            Select Case Type.ToUpper
                ...
                Case "VS10"
                    DS1 = PrcDS("USP_ISUD4064A_SEARCH_VS10_LINE", cn, strPara)
                    If GetDsRc(DS1) = 0 Then Exit Sub

                    Call READ_SPREAD(Vs10, DS1, "USP_ISUD4064A_SEARCH_VS10", "VS10", XROW)
                    DS1 = Nothing

                Case "VS20"
                    DS1 = PrcDS("USP_ISUD4064A_SEARCH_VS20_LINE", cn, strPara)
                    If GetDsRc(DS1) = 0 Then Exit Sub

                    Call READ_SPREAD(vS20, DS1, "USP_ISUD4064A_SEARCH_VS20", "VS20", XROW)
                    DS1 = Nothing
    ...
    Private Function DATA_SEARCH_VS10() As Boolean
        ...
        DS1 = PrcDS("USP_ISUD4064A_SEARCH_VS10", cn, txt_cdMainProcess.Code, txt_CustomerCode.Data, txt_cdSeason.Code, txt_Article.Data, txt_Line.Data, txt_SealNo.Data)

        If GetDsRc(DS1) = 0 Then
            SPR_PRO_NEW(Vs10, DS1, "USP_ISUD4064A_SEARCH_VS10", "VS10")
            Vs10.Enabled = True
            Exit Function
        End If
    ...
    Private Function DATA_SEARCH_VS20() As Boolean
        ...
        DS1 = PrcDS("USP_ISUD4064A_SEARCH_DATE_PIVOT", cn, txt_SdateEdate.text1, txt_SdateEdate.text2, txt_cdMainProcess.Code, txt_CustomerCode.Code, txt_cdSeason.Code, Value1, Value2)
        ...
        setDataCH(vS20, colvS20.SealNo, 0, "SealNo")
        setDataCH(vS20, colvS20.JobCard, 0, "JobCard")
        setDataCH(vS20, colvS20.cdDepartment, 0, "Code")
        setDataCH(vS20, colvS20.cdDepartmentName, 0, "Factory")
        setDataCH(vS20, colvS20.cdOSLineProd, 0, "Code")
        setDataCH(vS20, colvS20.cdOSLineProdName, 0, "Line")

        vS20.ActiveSheet.Columns(colvS20.JobCard).Visible = False
        vS20.ActiveSheet.Columns(colvS20.cdDepartment).Visible = False
        ...
        vS20.ActiveSheet.ColumnHeader.Cells(0, colvS20.SealNo).RowSpan = 2
        vS20.ActiveSheet.ColumnHeader.Cells(0, colvS20.cdDepartmentName).RowSpan = 2
...
#Region "Function"
    ...
    Private Function DATA_MOVE_K4064() As Boolean
        DATA_MOVE_K4064 = False
        Try

            If READ_PFK4064(W4064.Autokey) Then
                Call DATA_MOVE_DEFAULT()
                W4064.DateUpdate = Pub.DAT
                W4064.InchargeUpdate = Pub.SAW

                If REWRITE_PFK4064(W4064) = True Then

                End If

            Else
                W4064.DateInsert = Pub.DAT
                W4064.InchargeUpdate = Pub.SAW

                If WRITE_PFK4064(W4064) = True Then
                    isudCHK = True
                    DATA_MOVE_K4064 = True
                End If
    ...
    Private Function CheckExist(cdMainProcess As String, cdMainProcessSeq As String) As Boolean
        ...
        Dim RC As Integer
        Try
            rd = PrcDR("USP_ISUD4064A_SEARCH_VS00_CHECKEXIST", cn, cdMainProcess, cdMainProcessSeq)
            ...
            Select Case wJOB
                Case 1 ' Insert
                    ...
                    If checkisud = False Then
                        If DataCheck(Me, "PFK4064") = False Then MsgBoxP("Fill correct data !") : Exit Sub
                        Call KEY_COUNT()

                        If DATA_MOVE_K4064() = True Then
                ...
                Case 3 ' Update
                    If checkisud = False Then
                        ...
                        checkisud = True
                    Else
                        If DataCheck(Me, "PFK4064") = False Then Exit Sub
            ...
            Case Keys.Delete
                ...
                Select Case True
                    ...
                    Case sender Is vS20
                        ...
                        JobCard = getData(vS20, colvS20.JobCard, xROW)
                        cdDepartment = getData(vS20, colvS20.cdDepartment, xROW)
                        cdOSLineProd = getData(vS20, colvS20.cdOSLineProd, xROW)

                        If DELETE_PFK4064_PROCESS(JobCard, txt_cdMainProcess.Code, cdDepartment, cdOSLineProd) Then
    ...
    Private Sub Cms_2_ItemClicked(sender As Object, e As ToolStripItemClickedEventArgs) Handles Cms_2.ItemClicked
        If Cms_2.Items(0).Selected = True Then
            ...
            If READ_PFK4010(SealNo) Then
                ...
                If ISUD4064B.Link_ISUD4064B("1", D4010.DateStart, D4010.DateFinish, SealNo, txt_cdMainProcess.Code, Me.Tag) = False Then Exit Sub
