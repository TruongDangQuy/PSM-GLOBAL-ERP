...
#Region "Link"
    Public Function Link_ISUD4073G(job As Integer, cdFactory As String, cdMainProcess As String, cdLineProd As String, LineTno As String, Optional ByVal TAG As String = "") As Boolean
        Me.Tag = TAG
...
AgainPoint:
        ...
        'cmd_DEL.Visible = False
        'If Pub.DEVCHK = "1" And wJOB = 4 Then cmd_DEL.Visible = True
...
#Region "Search"
    ...
    Private Function DATA_SEARCH_VS_MALT() As Boolean
        ...
        Try
            ...
            DS1 = PrcDS("USP_ISUD4073F_SEARCH_VS_CP", cn, L4073.cdFactory,
                                                              L4073.cdMainProcess,
                                                              L4073.cdLineProd,
                                                              L4073.LineTno)
            ...
            If GetDsRc(DS1) = 0 Then
                ...
                SPR_PRO(vS_Malt, DS1, "USP_ISUD4073F_SEARCH_VS_CP", "VS_MALT")
                Exit Function
            End If
    ...
    Private Function DATA_SEARCH_VS2() As Boolean
        ...
        ComponentName = getData(vS_Malt, getColumIndex(vS_Malt, "ComponentName"), vS_Malt.ActiveSheet.ActiveRowIndex)

        ComponentName = ComponentName.Replace("'", "''")

        Try

            DS1 = PrcDS("USP_ISUD4073G_SEARCH_VS2", cn, L4073.cdFactory,
                                                              L4073.cdMainProcess,
                                                              L4073.cdLineProd,
                                                              L4073.LineTno,
                                                            ComponentName)
            ...
            If GetDsRc(DS1) = 0 Then
                SPR_PRO(vS2, DS1, "USP_ISUD4073G_SEARCH_VS2", "VS2")
...
next1:
        ...
        Dim Comsumption As Decimal

        For j = 0 To vS_Malt.ActiveSheet.RowCount - 1
            If getDataM(vS_Malt, getColumIndex(vS_Malt, "DCHK"), j) = "1" Then
                ComponentName = getData(vS_Malt, getColumIndex(vS_Malt, "ComponentName"), j)
                Comsumption = CDecp(getData(vS_Malt, getColumIndex(vS_Malt, "Consumption"), j))

                Call KEY_COUNT_K4175_BatchGroup()

                If Comsumption = 0 Then Comsumption = 1
                If FormatCut(ComponentName) <> "" Then

                    For i = Scol To vS1.ActiveSheet.ColumnCount - 1
                        If getDataM(vS1, i, 1) = "1" And CIntp(getDataM(vS1, i, 0)) >= 1 Then
                            Select Case txt_cdMainProcess.Code
                                Case "030"
                                    Call PrcExeNoError("USP_PFK4175_AUTO_BATCHNO_PRINTTING_SIZECHECK_QTYBASE_BATCHGROUP", cn, L4073.cdFactory, "", "", L4073.cdLineProd, L4073.LineTno, L4073.JobCard, (i - FixCol + 1).ToString("00"), CIntp(getDataM(vS1, i, 0)), txt_InchargePlan.Code, FormatSQL(ComponentName), CIntp(getDataM(vS1, i, 0)), W4175.BatchGroup)
                                Case "101"
                                    Call PrcExeNoError("USP_PFK4175_AUTO_BATCHNO_EMB_SIZECHECK_QTYBASE_BATCHGROUP", cn, L4073.cdFactory, "", "", L4073.cdLineProd, L4073.LineTno, L4073.JobCard, (i - FixCol + 1).ToString("00"), CIntp(getDataM(vS1, i, 0)) * Comsumption, txt_InchargePlan.Code, FormatSQL(ComponentName), CIntp(getDataM(vS1, i, 0)) * Comsumption, W4175.BatchGroup)
                                Case "102"
                                    Call PrcExeNoError("USP_PFK4175_AUTO_BATCHNO_HF_SIZECHECK_QTYBASE_BATCHGROUP", cn, L4073.cdFactory, "", "", L4073.cdLineProd, L4073.LineTno, L4073.JobCard, (i - FixCol + 1).ToString("00"), CIntp(getDataM(vS1, i, 0)), txt_InchargePlan.Code, FormatSQL(ComponentName), CIntp(getDataM(vS1, i, 0)), W4175.BatchGroup)
                            End Select
                        End If
    ...
    Private Sub vS_Malt_CellClick(sender As Object, e As CellClickEventArgs) Handles vS_Malt.CellClick
        ...
        If e.Column = getColumIndex(vS_Malt, "DCHK") Then vS_Malt.ActiveSheet.OperationMode = OperationMode.Normal
        If e.Column <> getColumIndex(vS_Malt, "DCHK") Then vS_Malt.ActiveSheet.OperationMode = OperationMode.SingleSelect
...
#End Region
    ...
    Public Sub PrintDocument1_PrintPage(sender As Object, e As Printing.PrintPageEventArgs) Handles PrintDocument1.PrintPage
        ...
        'Call TAG_PRINT_CUTTING_PANEL_NEW_PROD_TEST()
    End Sub
    ...
    Private Sub vS2_CellClick_1(sender As Object, e As CellClickEventArgs) Handles vS2.CellClick
