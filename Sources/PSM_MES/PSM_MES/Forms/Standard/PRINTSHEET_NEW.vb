...
Public Class PRINTSHEET_NEW
    ...
    '        chuoi2 = ""
    '        chuoi2 = GETCHUOI2_ShoesCode(Form_T)

    '        Dim DSSC As New DataSet
    '        Dim cmd As New SqlClient.SqlCommand

    '        If chuoi2 = "" Then ChkAdd = False Else ChkAdd = True

    '        If READ_PFK7106(Strings.Left(chuoi2, 6)) = False Then Exit Sub

    '        If ChkAdd = True And D7106.Customercode <> "000004" Then
    '            SQL = " SELECT K7106_ShoesCode,K7106_MCODE,K7106_ColorCODE,K7106_ColorName FROM PFK7106 WHERE K7106_ShoesParent = '" + Strings.Left(chuoi2, 6) + "' AND K7106_CheckUse = '1' "
    '            cmd = New SqlClient.SqlCommand(SQL, cn)
    '            DSSC = PrcDS(cmd, cn)

    '            If GetDsRc(DSSC) > 1 Then
    '                For i = 0 To GetDsRc(DSSC) - 2
    '                    DSReprt.Tables(0).Rows.Add("RND_SPEC2")
    '                    DSReprt.Tables(0).Rows.Add("RND_SPEC3")
    '                Next
    '            End If

    '        Else
    '            SQL = " SELECT K7106_ShoesCode,K7106_MCODE,K7106_ColorCODE,K7106_ColorName FROM PFK7106 WHERE K7106_ShoesCode = '" + Strings.Left(chuoi2, 6) + "' AND K7106_CheckUse = '1' "
    '            cmd = New SqlClient.SqlCommand(SQL, cn)
    '            DSSC = PrcDS(cmd, cn)
    ...
    '        DSReprt.Tables(0).Rows.Add("RND_SPEC4")
    ...
    '                Dim rd_Main As SqlClient.SqlDataReader
    '                Dim cmd_Main As New SqlClient.SqlCommand
    '                Dim NowVersion As String

    '                cmd_Main = New SqlClient.SqlCommand(SQL, cn)
    '                rd_Main = cmd_Main.ExecuteReader

    '                If rd_Main.HasRows Then
    '                    rd_Main.Read()
    '                    NowVersion = rd_Main("VERSION")
    '                End If
    '                rd_Main.Close()
    ...
    '                If My.Computer.FileSystem.FileExists(App_Path & "\Template\" & GetDsData(DSReprt, SoSheet, 0) & "_V" & NowVersion & ".xlsx") = False Then
    '                    myWebClient.DownloadFile(str, App_Path & "\Template\" & GetDsData(DSReprt, SoSheet, 0) & "_V" & NowVersion & ".xlsx")
    '                End If

    '                If SoSheet = 0 Then
    '                    vSPrint.OpenExcel(App_Path & "\Template\" & GetDsData(DSReprt, SoSheet, 0) & "_V" & NowVersion & ".xlsx", True)
    '                Else
    '                    vSPrint.ActiveSheet.OpenExcel(App_Path & "\Template\" & GetDsData(DSReprt, SoSheet, 0) & "_V" & NowVersion & ".xlsx", 0)
    '                End If

    '                'myWebClient.DownloadFile(str, App_Path & "\Report\" & GetDsData(DSReprt, SoSheet, 0) & ".xls")
    '                'If SoSheet = 0 Then
    '                '    vSPrint.OpenExcel(App_Path & "\Report" & "\" & GetDsData(DSReprt, SoSheet, 0) & ".xls", True)
    '                'Else
    '                '    vSPrint.ActiveSheet.OpenExcel(App_Path & "\Report" & "\" & GetDsData(DSReprt, SoSheet, 0) & ".xls", 0)
    '                'End If

    '                If SoSheet > 0 And SoSheet < DSReprt.Tables(0).Rows.Count - 1 Then
    '                    Dim xint As Integer

    '                    xint = Math.Ceiling(SoSheet / 2)
    '                    xint -= 1

    '                    If (SoSheet Mod 2) = 1 Then
    '                        chuoi2 = GetDsData(DSSC, xint, 0) + ";" + GetDsData(DSSC, xint, 0)
    '                    Else
    '                        chuoi2 = GetDsData(DSSC, xint, 0) + ";" + GetDsData(DSSC, xint, 0) + ";" + GetDsData(DSSC, xint, 0)
    '                    End If
    ...
    '                        If FormatCut(GetDsData(DSSC, xint, 2)) = "" Then
    '                            VALUENAME = GetDsData(DSSC, xint, 1) + "_" + GetDsData(DSSC, xint, 3) + "_" + "1"
    '                        Else
    '                            VALUENAME = GetDsData(DSSC, xint, 1) + "_" + GetDsData(DSSC, xint, 2) + "_" + "1"
    '                        End If

    '                    Else
    '                        If FormatCut(GetDsData(DSSC, xint, 2)) = "" Then
    '                            VALUENAME = GetDsData(DSSC, xint, 1) + "_" + GetDsData(DSSC, xint, 3) + "_" + "2"
    '                        Else
    '                            VALUENAME = GetDsData(DSSC, xint, 1) + "_" + GetDsData(DSSC, xint, 2) + "_" + "2"
    '                        End If

    '                    End If

    '                    vSPrint.ActiveSheet.SheetName = VALUENAME
    '                ElseIf SoSheet = 0 Then
    '                    vSPrint.ActiveSheet.SheetName = "COVER"
    '                Else
    '                    vSPrint.ActiveSheet.SheetName = "GROUP PART"
    '                End If
    ...
    '                If "RND_SPEC2" = GetDsData(DSReprt, SoSheet, 0) Then
    '                    For i = 9 To 68
    '                        If FormatCut(getData(vSPrint, 5, i)) = "" Then
    '                            vSPrint.ActiveSheet.Rows(i).Visible = False
    '                        End If
    '                    Next
    '                End If

    '                If "RND_SPEC3" = GetDsData(DSReprt, SoSheet, 0) Then
    '                    For i = 9 To 88
    '                        If FormatCut(getData(vSPrint, 5, i)) = "" Then
    '                            vSPrint.ActiveSheet.Rows(i).Visible = False
    '                        End If
    ...
    '                If "RND_SPEC4" = GetDsData(DSReprt, SoSheet, 0) Then
    '                    For i = 9 To 93
    '                        If FormatCut(getData(vSPrint, 3, i)) = "" Then
    '                            vSPrint.ActiveSheet.Rows(i).Visible = False
    '                        End If
    '                    Next
    '                End If
    ...
    Private Sub PRINTSHEET_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        Try
            ...
            If My.Computer.FileSystem.FileExists(App_Path & "\Template\JOB_SPEC" & "_V" & NowVersion & ".xlsx") = False Then
                myWebClient.DownloadFile(str, App_Path & "\Template\JOB_SPEC" & "_V" & NowVersion & ".xlsx")
            End If

            vSPrint.OpenExcel(App_Path & "\Template\JOB_SPEC" & "_V" & NowVersion & ".xlsx", True)
            ...
            For SoSheet = 0 To DSReprt.Tables(0).Rows.Count - 1
                ...
                Else
                    ...
                    i = vSPrint.ActiveSheetIndex
                    While i < vSPrint.Sheets.Count - 1
                        vSPrint.Sheets.RemoveAt(i)
                    End While

                    vSPrint.ActiveSheetIndex = vSPrint.Sheets.Count - 1
                    vSPrint.ActiveSheet.SheetName = "GROUP PART"
                End If
