Public Class ISUD4190A

#Region "Variable"
    Private wJOB As Integer

    Private _cdMainProcess As String
    Private _cdSubProcess As String
    Private _ProdDate As String
    Private _Autokey As String
...
#Region "Form"
    Public Function Link_ISUD40190A(job As Integer, cdSubProcess As String, ProdDate As String, Autokey As String, _
                                                                            JobCard As String, cdFactory As String, cdLineProd As String, Optional ByVal TAG As String = "") As Boolean
        Me.Tag = TAG

        _ProdDate = FormatCut(ProdDate)
        ...
        If READ_PFK7171(ListCode("SubProcess"), cdSubProcess) Then
            ...
            _cdMainProcess = D7171.cdBasicMaster
        End If

        If READ_PFK7171(ListCode("MainProcess"), D7171.cdBasicMaster) Then
...
#Region "Search"
    ...
    Private Function DATA_SEARCH_VS1() As Boolean
        ...
        Try
            DS1 = PrcDS("USP_ISUD4190A_SEARCH_VS1", cn, FormatCut(txt_ProdDate.Data), _
            ...
            If GetDsRc(DS1) = 0 Then
                ...
                SPR_PRO(vS1, DS1, "USP_ISUD4190A_SEARCH_VS1", "vS1")
...
#Region "Function"
    ...
    Private Sub DATA_LINE_DELETE(xrow As Integer)
        Try
            ...
            Dim Autokey As Integer = getData(vS1, getColumIndex(vS1, "KEY_Autokey"), xrow)


            Call PrcExeNoError("USP_ISUD4190A_SEARCH_VS1_DELETE", cn, Autokey)
    ...
    Private Sub DATA_INIT()
        Try
            txt_ProdDate.Data = FSDate(_ProdDate)
            txt_ProdDate.Code = FormatCut(_ProdDate)
            txt_Autokey.Data = _Autokey
...
#Region "Event"
    ...
    Private Sub vSChange(xROW As Integer)
        ...
        Dim QtyProduction As Double = FormatCut(getData(vS1, getColumIndex(vS1, "QtyProduction"), xROW))
        Dim Autokey As Integer = getData(vS1, getColumIndex(vS1, "KEY_Autokey"), xROW)
      

        Call PrcExeNoError("USP_ISUD4190A_SEARCH_VS1_UPDATE", cn, Autokey, QtyProduction)
    ...
    Private Sub txt_Barcode_txtTextKeyDown(sender As Object, e As KeyEventArgs) Handles txt_Barcode.txtTextKeyDown
        If e.KeyCode = Keys.Enter Then
            ...
            If READ_PFK4010_SlnoD(txt_Barcode.Code) Then
                _JobCard = D4010.JobCard
            Else
                If Len(txt_Barcode.Code) <> 9 Then
                    txt_Mess.Text = "LỖI LỆNH SẢN XUẤT!"
                ...
                _JobCard = txt_Barcode.Code
            End If

            If READ_PFK4010(_JobCard) Then

                DS1 = PrcDS("USP_ISUD4190A_SEARCH_VS1_INSERT", cn, FormatCut(txt_ProdDate.Data),
                            txt_cdFactory.Code,
                            txt_cdLineProd.Code,
                            txt_cdMainProcess.Code,
                            txt_cdSubProcess.Code,
                            txt_InchargeProdution.Code,
                            _JobCard,
                            D4010.QtyJob)


                If GetDsData(DS1, 0, 0).Contains("THÀNH CÔNG") = False Then
            ...
            Else
                ...
                txt_Mess.Text = "LỆNH SẢN XUẤT KHÔNG TỒN TẠI!"
                txt_Mess.ForeColor = Color.Red
