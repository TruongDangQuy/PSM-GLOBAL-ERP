...
#Region "Form_load"
    ...
    Private Sub DATA_INIT()
        ssp_WHERE.txt_SdateEdate.text1 = Strings.Left(Pub.DAT, 4) & "0101"
...
#Region "Data_search"
    ...
    Private Function DATA_SEARCH_VS1(Optional ByVal xsort As String = "1") As Boolean
        ...
        Try
            DS1 = PrcDS("USP_PFP13153_SEARCH_VS1", cn, ssp_WHERE.gData_SDate,
            ...
            If GetDsRc(DS1) = 0 Then
                SPR_PRO_NEW(Vs1, DS1, "USP_PFP13153_SEARCH_VS1", "Vs1")
    ...
    Private Function DATA_SEARCH_VS2(Optional ByVal xsort As String = "1") As Boolean
        ...
        Try
            DS1 = PrcDS("USP_PFP13153_SEARCH_VS2", cn, ssp_WHERE.gData_SDate,
                                                         ssp_WHERE.gData_EDate)


            If GetDsRc(DS1) = 0 Then
                SPR_PRO_NEW(VS2, DS1, "USP_PFP13153_SEARCH_VS2", "VS2")
...
#End Region
    ...
    Private Sub vSChange(xROW As Integer)
        Try
            ...
            Dim OrderNo As String = getData(Vs1, getColumIndex(Vs1, "OrderNo"), xROW)
            Dim OrderNoSeq As String = getData(Vs1, getColumIndex(Vs1, "OrderNoSeq"), xROW)
            Dim DateCL As String = FormatCut(getData(Vs1, getColumIndex(Vs1, "DateCL"), xROW))
            Dim QtyOrder As String = getData(Vs1, getColumIndex(Vs1, "QtyOrder"), xROW)

            Dim LicenseName As String = getData(Vs1, getColumIndex(Vs1, "LicenseName"), xROW)
            Dim TestStandard As String = getData(Vs1, getColumIndex(Vs1, "TestStandard"), xROW)
            Dim TestNo As String = getData(Vs1, getColumIndex(Vs1, "TestNo"), xROW)
            Dim DateReport As String = FormatCut(getData(Vs1, getColumIndex(Vs1, "DateReport"), xROW))

            Dim PriceOrderFOB As String = CDecp(getData(Vs1, getColumIndex(Vs1, "PriceOrderFOB"), xROW))
            Dim CheckFOB As String = getDataM(Vs1, getColumIndex(Vs1, "CheckFOB"), xROW)

            Dim TotalAMTFOB As String = PriceOrderFOB * CDecp(getData(Vs1, getColumIndex(Vs1, "QtyOrder"), xROW))


            Dim StrValue As String

            StrValue = PrcExeNoError_Value("USP_PFK1312_UPDATE_FOB", cn, OrderNo, OrderNoSeq, DateCL, LicenseName, TestStandard, TestNo, DateReport, Pub.SAW, Pub.DAT, PriceOrderFOB, CheckFOB)

            setData(Vs1, getColumIndex(Vs1, "Result"), xROW, StrValue)
    ...
    Private Sub PeaceButton1_Click(sender As Object, e As EventArgs) Handles cmd_Save.Click
        Try
            Dim xROW As Integer
            Dim dchk As String

            If ptc_001.SelectedIndex <> 0 Then MsgBoxP("Wrong TAB!") : Exit Sub

            For xROW = 0 To Vs1.ActiveSheet.RowCount - 1
                dchk = getDataM(Vs1, getColumIndex(Vs1, "DCHK"), xROW)
            ...
            Call MsgBoxP("SAVE BATCH successfully !", vbInformation)

        Catch ex As Exception
            MsgBoxP("Error in SAVE BATCH!", vbCritical)
        End Try
    End Sub
End Class