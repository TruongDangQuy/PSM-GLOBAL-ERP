...
#Region "Link"
    Public Function Link_ISUD4070C(job As Integer, cdFactory As String, cdMainProcess As String, MachineCode As String, MachineTno As String, Optional ByVal TAG As String = "") As Boolean
        Me.Tag = TAG
            ...
            Select Case job
                ...
                Case Else
                    If READ_PFK4070(cdFactory, cdMainProcess, MachineCode, MachineTno) Then
                        If D4070.QtyProd > 0 Or D4070.CheckComplete <> "1" Then cmd_DEL.Visible = False : cmd_OK.Visible = True : wJOB = 2
                    Else
                        Exit Function
                    End If
    ...
    Public Function Link_ISUD4070C_JOBCARD(job As Integer, JobCard As String, cdFactory As String, cdMainProcess As String, MachineCode As String, MachineTno As String, Optional ByVal TAG As String = "") As Boolean
        Me.Tag = TAG
        D4070.MachineCode = MachineCode
        D4070.cdFactory = cdFactory
        D4070.cdMainProcess = cdMainProcess
        ...
        Try
            ...
            Select Case job
                ...
                Case Else
                    If READ_PFK4070(cdFactory, cdMainProcess, MachineCode, MachineTno) Then
                        If D4070.QtyProd > 0 Then cmd_DEL.Visible = False : cmd_OK.Visible = True : wJOB = 2
                    Else
                        Exit Function
                    End If
            ...
            Link_ISUD4070C_JOBCARD = isudCHK
...
#Region "Search"
    ...
    Private Function DATA_SEARCH_VS0() As Boolean
        ...
        Try
            ...
            DS1 = PrcDS("USP_ISUD4070C_SEARCH_VS0", cn, L4070.cdFactory,
                                                              L4070.cdMainProcess,
                                                              L4070.MachineCode,
                                                              L4070.MachineTno)
            ...
            If GetDsRc(DS1) = 0 Then
                VS0.Enabled = True
                SPR_PRO(vS0, DS1, "USP_ISUD4070C_SEARCH_VS0", "VS0")
                Exit Function
            End If
    ...
    Private Function DATA_SEARCH_VS1() As Boolean
        ...
        Try
            ...
            DS1 = PrcDS("USP_ISUD4070C_SEARCH_vS1", cn, L4070.cdFactory,
                                                              L4070.cdMainProcess,
                                                              L4070.MachineCode,
                                                              L4070.MachineTno)
            ...
            If GetDsRc(DS1) = 0 Then
                ...
                SPR_PRO(vS1, DS1, "USP_ISUD4070C_SEARCH_vS1", "Vs1")
                Exit Function
            End If
            ...
            If READ_PFK4070(L4070.cdFactory, L4070.cdMainProcess, L4070.MachineCode, L4070.MachineTno) Then
                Call VsSizeRangeNew(vS1, "USP_VS_SIZERANGE_JOBBASE_MOLD", "SizeQty01", D4070.JobCard)
            End If

            'Call VsSizeRangeNew(vS1, "USP_VS_SIZERANGE_JOBBASE", "SizeQty01", txt_JobCard.Data)
            vS1.ActiveSheet.RowCount += 3

            SPR_CHECKBOXROW(vS1, 2)

            Scol = getColumIndex(vS1, "SizeQty01")
            FixCol = getColumIndex(vS1, "SizeQty01")

            Call READ_PFK7101_NAME(txt_CustomerName.Data)
            If D7101.ParaNo1 = 0 Then D7101.ParaNo1 = 5

            For i = Scol To vS1.ActiveSheet.ColumnCount - 1
                If CIntp(getData(vS1, i, 0)) > 0 Then
                    setData(vS1, i, 3, D7101.ParaNo1) ' getData(vS1, i, 0))
                    setData(vS1, i, 4, 1)
                End If
            Next

            setData(vS1, getColumIndex(vS1, "Title"), 3, "3. QTY/1 BATCH")
            setData(vS1, getColumIndex(vS1, "Title"), 4, "4. BATCH QTY")
    ...
    Private Function DATA_SEARCH_VS2() As Boolean
        ...
        Try
            DS1 = PrcDS("USP_ISUD4070C_SEARCH_VS2_BATCHGROUP", cn, L4070.cdFactory,
                                                              L4070.cdMainProcess,
                                                              L4070.MachineCode,
                                                              L4070.MachineTno, getData(vS0, getColumIndex(vS0, "SizeName"), vS0.ActiveSheet.ActiveRowIndex), chk_No.CheckState.ToString)

            If GetDsRc(DS1) = 0 Then
                vS2.Enabled = True
                SPR_SET(vS2, , , , OperationMode.Normal)
                SPR_PRO(vS2, DS1, "USP_ISUD4070C_SEARCH_VS2", "VS2")
                Exit Function
            End If
    ...
    Private Function DATA_SEARCH_HEAD() As Boolean
        ...
        Try
            ...
            DS1 = PrcDS("USP_ISUD4070C_SEARCH_HEAD", cn, L4070.cdFactory,
                                                         L4070.cdMainProcess,
                                                         L4070.MachineCode,
                                                         L4070.MachineTno)
            ...
            If READ_PFK4010(L4070.JobCard) Then
                If READ_PFK1312(D4010.WorkOrder, D4010.WorkOrderSeq) Then
                    If READ_PFK7106(D1312.ShoesCode) Then
                        txt_ColorNamePrint.Data = D7106.Note8

                        If FormatCut(txt_ColorNamePrint.Data) = "" Then
                            If READ_PFK7109_OUTSOLE_NAME(D1312.ShoesCode) Then
                                txt_ColorNamePrint.Data = D7109.ColorName
                            End If
                        End If
            ...
            Call K4070_CheckProduction(GetDsData(DS1, 0, "PartProduction"))
            Call K4070_CheckTrigger(GetDsData(DS1, 0, "CheckTrigger"))
    ...
    Private Function DATA_SEARCH_HEAD_TAB1() As Boolean
        ...
        '    DS1 = PrcDS("USP_ISUD4070C_SEARCH_sTAB1_HEAD", cn, L4070.JobCard)
    ...
    Private Function DATA_SEARCH01() As Boolean
        DATA_SEARCH01 = False
        Try
            DS1 = PrcDS("USP_ISUD4070C_SEARCH_vS1", cn, L4070.cdFactory, L4070.cdMainProcess, L4070.MachineCode, L4070.MachineTno)
            ...
            If READ_PFK4070(L4070.cdFactory, L4070.cdMainProcess, L4070.MachineCode, L4070.MachineTno) Then
                ...
                For i = Scol To vS1.ActiveSheet.ColumnCount - 1
                    If READ_PFK4071(L4070.cdFactory, L4070.cdMainProcess, L4070.MachineCode, L4070.MachineTno, (i - FixCol + 1).ToString("00")) Then
                        ...
                        If W4071.QtyProd > 0 Then MsgBoxP("Already Prod!") : Exit Function
                    End If
                Next
    ...
    Private Function DATA_SEARCH_VS1_INSERT() As Boolean
        ...
        Try
            ...
            DS1 = PrcDS("USP_ISUD4070C_SEARCH_VS1_INSERT", cn, L4070.JobCard)
            ...
            Call READ_SPREAD(vS1, DS1, "USP_ISUD4070C_SEARCH_vS1", "VS1", xrow)
    ...
    Private Sub K4070_CheckTrigger(Value As String)
        rad_CheckTrigger1.Checked = True
        If Value = "1" Then rad_CheckTrigger1.Checked = True : Exit Sub
        If Value = "2" Then rad_CheckTrigger2.Checked = True : Exit Sub
        If Value = "3" Then rad_CheckTrigger3.Checked = True : Exit Sub
        If Value = "4" Then rad_CheckTrigger4.Checked = True : Exit Sub
    End Sub
    ...
    Private Sub K4070_CheckTrigger()
        W4070.CheckTrigger = "1"
        If rad_CheckTrigger1.Checked = True Then W4070.CheckTrigger = "1" : Exit Sub
        If rad_CheckTrigger2.Checked = True Then W4070.CheckTrigger = "2" : Exit Sub
        If rad_CheckTrigger3.Checked = True Then W4070.CheckTrigger = "3" : Exit Sub
        If rad_CheckTrigger4.Checked = True Then W4070.CheckTrigger = "4" : Exit Sub

    End Sub

    Private Sub K4070_CheckProduction()
        W4070.PartProduction = "1"
        If chk_PartProduction1.Checked = True Then W4070.PartProduction = "1" : Exit Sub
        If chk_PartProduction2.Checked = True Then W4070.PartProduction = "2" : Exit Sub
        If chk_PartProduction3.Checked = True Then W4070.PartProduction = "3" : Exit Sub

    End Sub

    Private Sub K4070_CheckProduction(Value As String)
        chk_PartProduction1.Checked = True
        If Value = "1" Then chk_PartProduction1.Checked = True : Exit Sub
        If Value = "2" Then chk_PartProduction2.Checked = True : Exit Sub
        If Value = "3" Then chk_PartProduction3.Checked = True : Exit Sub
    End Sub
    ...
    Private Function DATA_CHECKCOMPLETE() As Boolean
        ...
        Try
            ...
            If READ_PFK4070(L4070.cdFactory, L4070.cdMainProcess, L4070.MachineCode, L4070.MachineTno) = True Then
                W4070 = D4070
                W4070.Remark = txt_ColorNamePrint.Data
                ...
                If READ_PFK1312(D4070.WorkOrder, D4070.WorkOrderSeq) Then
                    If READ_PFK7106(D1312.ShoesCode) Then
                        D7106.MoldCode = txt_MoldCode.Code
                        D7106.OutsoleColor = txt_OutsoleColor.Data
                        D7106.OutsoleColorCode = txt_OutsoleColorCode.Data
...
#Region "Event"
    ...
    Private Sub cmd_Generate_Click(sender As Object, e As EventArgs) Handles cmd_Generate.Click
        If FormatCut(txt_MoldCode.Code) = "" Then MsgBoxP("Mold Pls!") : Exit Sub
        ...
        For i = Scol To vS1.ActiveSheet.ColumnCount - 1
            If getDataM(vS1, i, 2) = "1" And CIntp(getDataM(vS1, i, 4)) >= 1 Then

                If READ_PFK4071(L4070.cdFactory, L4070.cdMainProcess, L4070.MachineCode, L4070.MachineTno, (i - FixCol + 1).ToString("00")) Then



                    For j = 0 To CIntp(getData(vS1, i, 4)) - 1

                        If txt_cdMainProcess.Code = "110" Then
                            Call PrcExeNoError("USP_PFK4075_AUTO_BATCHNO_STITCHING_SZNO_MACHINE_BATCHQTY_110_JOBCARD", cn, D4071.cdFactory,
                                       D4071.MachineCode,
                                       D4071.MachineTno,
                                       "",
                                       "",
                                       ...
                                       D4071.WorkOrderSeq,
                                       D4071.Szno,
                                       CIntp(getData(vS1, i, 3)),
                                       txt_InchargePlan.Code,
                                       txt_JobCard.Data)
                        Else
                            Call PrcExeNoError("USP_PFK4075_AUTO_BATCHNO_STITCHING_SZNO_MACHINE_BATCHQTY_JOBCARD", cn, D4071.cdFactory,
                                         D4071.MachineCode,
                                         D4071.MachineTno,
                                         "",
                                         "",
    ...
    Private Sub vS2_KeyDown(sender As Object, e As KeyEventArgs) Handles vS2.KeyDown
        If e.KeyCode = Keys.Delete Then
            ...
            BatchNo = getData(vS2, getColumIndex(vS2, "BatchNo"), vS2.ActiveSheet.ActiveRowIndex)
            ...
            If READ_PFK4075_BatchNo(BatchNo) = True Then
                W4075 = D4075
                If W4075.QtyProduction > 0 Then MsgBoxP("Already production!") : Exit Sub
    ...
    Public Sub PrintDocument1_PrintPage(sender As Object, e As Printing.PrintPageEventArgs) Handles PrintDocument1.PrintPage
        gpp = e.Graphics
        If chk_smallBarcode.Checked Then
            Call TAG_PRINT_OS_PANEL_4Barcode()
        Else
    ...
    Private Sub cmd_BarCode_Click(sender As Object, e As EventArgs) Handles cmd_BarCode.Click
        ...
        If FormatCut(txt_ColorNamePrint.Data) = "" Then
            MsgBoxP("Vui lòng nhập tên màu viết tắt! Color Name Print Pls ! ")
            Setfocus(txt_ColorNamePrint)
            Exit Sub
        End If
        ...
        Try
            ...
            If Msg_Result = vbYes Then
                ...
                Call READ_PFK7155(txt_MachineCode.Code)
                ...
                If chk_smallBarcode.Checked Then
                    ...
                    For i = 0 To vS2.ActiveSheet.RowCount - 1 Step 4
                        ...
                        If getDataM(vS2, getColumIndex(vS2, "DCHK"), i) = "1" Then
                            ...
                            DATA_MOVE_BARCODE_1_2(getData(vS2, getColumIndex(vS2, "Batchno"), i), getData(vS2, getColumIndex(vS2, "Batchno"), i + 1))
                            DATA_MOVE_BARCODE_3_4(getData(vS2, getColumIndex(vS2, "Batchno"), i + 2), getData(vS2, getColumIndex(vS2, "Batchno"), i + 3))
    ...
    Public Sub DATA_MOVE_BARCODE_1_2(Batchno1 As String, Batchno2 As String)
        If READ_PFK4075_BatchNo(Batchno1) Then
            ...
            OSPANEL1.ColorName = txt_ColorNamePrint.Data ' txt_OutsoleColorCode.Data & "-" & txt_LogoColorCode.Data
            ...
            OSPANEL1.OSColorCode = txt_OutsoleColorCode.Data
            OSPANEL1.OSColorName = txt_OutsoleColor.Data

            OSPANEL1.LGColorCode = txt_LogoColorCode.Data
            OSPANEL1.LGColorName = txt_LogoColor.Data

            OSPANEL1.Machine = D7155.VerticalPosition & "-" & D7155.HorizontalPosition
        ...
        If READ_PFK4075_BatchNo(Batchno2) Then
            ...
            OSPANEL2.ColorName = txt_ColorNamePrint.Data ' txt_OutsoleColorCode.Data & "-" & txt_LogoColorCode.Data
            ...
            OSPANEL2.Machine = D7155.VerticalPosition & "-" & D7155.HorizontalPosition
        End If
    End Sub

    Public Sub DATA_MOVE_BARCODE_3_4(Batchno1 As String, Batchno2 As String)
        ...
        If READ_PFK4075_BatchNo(Batchno1) Then
            ...
            OSPANEL3.DatePrint = Pub.DAT
            OSPANEL3.ColorName = txt_ColorNamePrint.Data ' txt_OutsoleColorCode.Data & "-" & txt_LogoColorCode.Data

            OSPANEL3.SizeName = D4075.SizeName

            OSPANEL3.SlNo = txt_SlNoD.Data
            OSPANEL3.MoldName = txt_MoldCode.Data

            OSPANEL3.QtySize = D4075.QtyBatch
            OSPANEL3.Barcode = D4075.BatchNo

            OSPANEL3.OSColorCode = txt_OutsoleColorCode.Data
            OSPANEL3.OSColorName = txt_OutsoleColor.Data

            OSPANEL3.LGColorCode = txt_LogoColorCode.Data
            OSPANEL3.LGColorName = txt_LogoColor.Data

            OSPANEL3.Machine = D7155.VerticalPosition & "-" & D7155.HorizontalPosition
        End If
        ...
        If READ_PFK4075_BatchNo(Batchno2) Then
            ...
            OSPANEL4.DatePrint = Pub.DAT
            OSPANEL4.ColorName = txt_ColorNamePrint.Data ' txt_OutsoleColorCode.Data & "-" & txt_LogoColorCode.Data

            OSPANEL4.SizeName = D4075.SizeName

            OSPANEL4.SlNo = txt_SlNoD.Data
            OSPANEL4.MoldName = txt_MoldCode.Data

            OSPANEL4.QtySize = D4075.QtyBatch
            OSPANEL4.Barcode = D4075.BatchNo

            OSPANEL4.OSColorCode = txt_OutsoleColorCode.Data
            OSPANEL4.OSColorName = txt_OutsoleColor.Data

            OSPANEL4.LGColorCode = txt_LogoColorCode.Data
            OSPANEL4.LGColorName = txt_LogoColor.Data

            OSPANEL4.Machine = D7155.VerticalPosition & "-" & D7155.HorizontalPosition
        End If
    End Sub
    ...
    Private Sub rad_CheckTrigger4_CheckedChanged(sender As Object, e As EventArgs) Handles rad_CheckTrigger4.CheckedChanged
        Try
            ...
            If READ_PFK4070(L4070.cdFactory, L4070.cdMainProcess, L4070.MachineCode, L4070.MachineTno) = True Then
                ...
                If REWRITE_PFK4070_TRIGGER(W4070) = True Then
                    isudCHK = True
                End If
        ...
        End Try
    End Sub
    Private Sub rad_CheckTrigger3_CheckedChanged(sender As Object, e As EventArgs) Handles rad_CheckTrigger3.CheckedChanged
        Try
        ...
        End Try
    End Sub
    Private Sub rad_CheckTrigger2_CheckedChanged(sender As Object, e As EventArgs) Handles rad_CheckTrigger2.CheckedChanged
        Try
        ...
        End Try
    End Sub
    Private Sub rad_CheckTrigger1_CheckedChanged(sender As Object, e As EventArgs) Handles rad_CheckTrigger1.CheckedChanged
        Try
    ...
    Private Sub vS0_KeyDown(sender As Object, e As KeyEventArgs) Handles vS0.KeyDown
        If e.KeyCode = Keys.F Then
            ...
            Msg_Result = MsgBoxP("TAG PRINT ALL?", vbYesNo)

            If Msg_Result = vbYes Then
                If chk_smallBarcode.Checked Then

                    For xrow = 0 To vS0.ActiveSheet.RowCount - 1

                        vS0.ActiveSheet.ActiveRowIndex = xrow

                        txt_BatchGroup.Data = getData(vS0, getColumIndex(vS0, "Key_BatchGroup"), xrow)
...
#End Region
    ...
    Private Sub cmd_ColorUpdate_Click(sender As Object, e As EventArgs) Handles cmd_ColorUpdate.Click
        MessSer = MsgBox("Do you want to update barcode print name ? Bạn có muốn cập nhật tên để in barcode ?", vbYesNo)
        ...
        If READ_PFK4010(txt_JobCard.Data) Then
            If READ_PFK1312(D4010.WorkOrder, D4010.WorkOrderSeq) Then
                If READ_PFK7106(D1312.ShoesCode) Then
                    ...
                    If FormatCut(D7106.Note8) <> "" Then

                        MessSer = MsgBox("Already print name. Do yu keep change ? Đã thay đổi tên rồi. Bạn vẫn tiếp tục ?", vbYesNo)
                        If MessSer <> vbYes Then Exit Sub
                    ...
                    strOldName = D7106.Note8

                    If Len(txt_ColorNamePrint.Data) > 0 Then D7106.Note8 = Strings.Left(txt_ColorNamePrint.Data, 50)

                   
                    If REWRITE_PFK7106(D7106) Then
                        D9700_CLEAR()
                        D9700.ActionName = wJOB & " thay đổi colorname của print " & strOldName & " - " & D7106.Note8
                        D9700.DateCreate = Pub.DAT
                        D9700.UserCode = Pub.SAW
                        D9700.FormName = Me.FindForm.Name
                        D9700.Reference = D7106.ShoesCode
